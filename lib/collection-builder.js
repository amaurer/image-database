...
exports.build = function(files, done){
	...
	for (i = files.length - 1; i >= 0; i--) {
		...
		exports.collectionByFolderStructureDecorator(collection, x, x)
...
exports.decorateImageGroup = function(fileMetaData, imageGroup, fullPath){
	...
	if(fileMetaData.pos == null){
		...
		imageGroup[imageSizeString][Number(fileMetaData.pos) - 1] = fileMetaData.file
	}
	temp = fullPath.split(_fileSep)
	temp.splice(temp.length-1, 1)
	imageGroup.path = _fileSep + temp.join(_fileSep)
}

exports.createImageGroup = function(fileMetaData, parent){
	return {
...
exports.collectionByFolderStructureDecorator = function(collection, file, fullPath){
	var fileSplit = file.split(_fileSep) // root, section, group, name
	var temp
	...
	if(fileSplit.length === 2){
		if(collection[group] == null) collection[group] = {}
			...
			if(collection[group][temp.name] == null) collection[group][temp.name] = exports.createImageGroup(temp, fileSplit[0])
			exports.decorateImageGroup(temp, collection[group][temp.name], fullPath)
	} else if(fileSplit.length > 2){
		if(collection[group] == null) collection[group] = {}
		exports.collectionByFolderStructureDecorator(collection[group], fileSplit.splice(1).join(_fileSep), fullPath)
	}
}
...
exports.removeTrailingSlash = function(collectionFilter){
	if(collectionFilter.substr(0,1) === _fileSep) collectionFilter = collectionFilter.substr(1)
	if(collectionFilter.substr(collectionFilter.length-1,1) === _fileSep) collectionFilter = collectionFilter.substr(0,collectionFilter.length-1)
	return collectionFilter
}
exports.find = function(collection, collectionFilter){
	collectionFilter = exports.removeTrailingSlash(collectionFilter)
	var filters = collectionFilter.split(_fileSep)
